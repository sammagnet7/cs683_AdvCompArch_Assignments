...
public:
    ...
    // custom counters
    int phase_progress = 0; // stores progress in current phase
    bool best_pf_decided = false;
    ...
    // Lateness counter
    uint64_t thr_pf_late, thr_pf_late_li;
    // Pollution counter
    uint64_t thr_pf_pollution_total, thr_pf_pollution_total_li, thr_demand_miss_total, thr_demand_miss_total_li, thr_eviction_count;
    // Bloom filter
    char pollution_filter[4096] = {0};
    // Throttling parameters
    double A_high = 0.75, A_low = 0.4, T_lateness = 0.01, T_pollution = 0.005;
        ...
        : NAME(v1), NUM_SET(v2), NUM_WAY(v3), NUM_LINE(v4), WQ_SIZE(v5), RQ_SIZE(v6), PQ_SIZE(v7), MSHR_SIZE(v8)
    {
        ...
        thr_demand_miss_total_li = 0;
        thr_pf_pollution_total = 0;
        thr_pf_pollution_total_li = 0;
    ...
    // functions
    void get_current_prefetcher(uint64_t pf_useful, uint64_t pf_lower_level);
    int set_index(uint64_t address);
    int unset_index(uint64_t address);
    char polluting_prefetch(uint64_t address);
    void throttler_operate();
