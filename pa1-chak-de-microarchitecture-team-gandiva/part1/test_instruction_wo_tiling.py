...
def plot_results(matrix_size, instr_over_matrix, tile_size):
    ...
    plt.title('Number of Instructions vs Matrix Size for Different Tile Sizes')
    ...
    for i in range(n_bars):
        for j in range(len(matrix_size)):
            plt.text(bar_positions[i][j], instr_over_matrix[j][i] + max(max(instr_over_matrix)) * 0.05, 
                 str(instr_over_matrix[j][i]), ha='center', va='bottom', rotation='vertical', fontsize=18)
    ...
    for m in matrix_size:    
        ...
        for b in tile_size:
                if(b==0):
                    ...
                    instr_over_block.append( round(run_perf_stat('prefetch', m, b),2) )
